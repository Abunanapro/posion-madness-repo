...
public class DarkFrogBowlRightclickedOnBlockProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		ItemStack frogtype = ItemStack.EMPTY;
		frogtype = new ItemStack(PoisonMadnessModItems.DARK_FROG_BOWL.get());
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == frogtype.getItem()) {
			...
			if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == Blocks.AIR) {
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = PoisonMadnessModEntities.POISON_FROG_DARK.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
				...
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = PoisonMadnessModEntities.POISON_FROG_DARK.get().spawn(_level, BlockPos.containing(x, y + 1, z), MobSpawnType.MOB_SUMMONED);
					if (entityToSpawn != null) {
						entityToSpawn.setYRot(world.getRandom().nextFloat() * 360F);
					}
				}
